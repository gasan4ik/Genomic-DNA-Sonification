#######################################################################
#
# This script maps the monophonic output from sonification to a three-part
# flute–viola–bassoon trio. Each part is assigned to its own pitch range using
# octave wrapping, then snapped to the C major scale to preserve tonal clarity.
# A fixed random seed ensures reproducible harmonic intervals, and a clear
# dynamic hierarchy (flute > viola > bassoon) optimizes balance and listener
# comfort for the trio texture analyzed in the manuscript.
#
#
...
OUTPUT_MIDI = "trio_output_refined.mid"
...
BASSOON_RANGE = (48, 55)    # C3–G3 (C3/G3 region)
...
for i, (raw_note, raw_vel) in enumerate(melody):
    ...
    # Slightly softer than flute
    viola_vel = max(30, int(raw_vel * 0.8))
    ...
    # Softer, supporting background
    bassoon_vel = max(25, int(raw_vel * 0.5))
